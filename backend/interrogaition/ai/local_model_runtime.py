...
DEFAULT_BRIDGE_BASE_URL = "http://127.0.0.1:8080/v1"
DEFAULT_BRIDGE_MODEL = "interrogaition-bridge"
...
class LocalModelRuntimeConfig:
    ...
    def effective_provider(self) -> str:
        if self.provider == "ollama" and self.real_model_enabled:
            return "ollama"
        if self.provider == "bridge" and self.real_model_enabled:
            return "bridge"
        return "deterministic"

    @property
    def bridge_api_key_configured(self) -> bool:
        return bool(self.bridge_api_key)
    ...
    def restrictions(self) -> tuple[str, ...]:
        ...
        elif self.live_output_enabled and self.effective_provider == "bridge":
            restrictions.append(
                "Live grounded suggestions use the configured bridge for developer experiments.",
            )
        else:
            restrictions.append(
                "Grounded suggestions use the deterministic local assistant unless gated live output is enabled.",
            )
        if self.provider in {"ollama", "bridge"} and not self.real_model_enabled:
            restrictions.append(f"{self.provider} is configured but real model execution is disabled.")
        if self.provider in {"ollama", "bridge"} and self.real_model_enabled and not self.live_output_enabled:
            restrictions.append(
...
def load_local_model_runtime_config(
    ...
    if provider not in {"deterministic", "ollama", "bridge"}:
        provider = "deterministic"

    bridge_model = values.get("INTERROGAITION_BRIDGE_MODEL", DEFAULT_BRIDGE_MODEL).strip()
    ...
    return LocalModelRuntimeConfig(
        ...
        bridge_base_url=values.get("INTERROGAITION_BRIDGE_BASE_URL", DEFAULT_BRIDGE_BASE_URL).strip()
        or DEFAULT_BRIDGE_BASE_URL,
        bridge_api_key=values.get("INTERROGAITION_BRIDGE_API_KEY", "").strip(),
        timeout_seconds=_parse_int(
            values.get("INTERROGAITION_MODEL_TIMEOUT_SECONDS")
            or values.get("INTERROGAITION_OLLAMA_TIMEOUT_SECONDS"),
            default=120,
        ),
    ...
    if execute_real:
        if config.provider not in {"ollama", "bridge"}:
            return LocalModelSmokeResult(
                ...
                model=config.configured_model,
                real_model_invoked=False,
                detail="Real smoke requires provider=ollama or provider=bridge.",
            )
        ...
        if model_client is not None:
            client = model_client
        elif config.provider == "bridge":
            client = BridgeModelClient(
                model=config.configured_model,
                base_url=config.bridge_base_url,
                api_key=config.bridge_api_key,
                timeout_seconds=config.timeout_seconds,
